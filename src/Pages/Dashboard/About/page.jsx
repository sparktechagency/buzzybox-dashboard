...
function AboutPage() {
  ...
  const [addAboutUs] = useCreateAboutUsMutation();
  ...
  const handleFormSubmit = async (values) => {
    ...
    if (values.aboutImage?.file?.originFileObj)
      formData.append("aboutImage", values.aboutImage?.file?.originFileObj);
    ...
    if (isEditing) {
      ...
      try {
        const res = await addAboutUs({ payload: formData }).unwrap();
        if (res.success) {
  ...
  const [deleteCategory] = useDeleteAboutUsMutation();
  ...
  const formatedAboutData = aboutsData?.map((item, index) => ({
    _id: item?._id,
    key: index + 1,
    serial: index + 1,
    title: item?.title,
      ...
      >
        ...
        >
          <Form form={form} layout="vertical" onFinish={handleFormSubmit}>
            ...
            <Form.Item label="Reverse Content" name="reverse" defaultChecked>
              <Switch />
            </Form.Item>
